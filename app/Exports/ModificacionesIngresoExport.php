...
class ModificacionesIngresoExport implements FromView, ShouldAutoSize, WithEvents
{
    public $ano;
    public $mes;
    public $tipo;
    ...
    public function __construct($ano, $mes, $tipo, $ue)
    {
        $this->ano = $ano;
        $this->mes = $mes;
        $this->tipo = $tipo;
    ...
    {
        $body = ModificacionesRepositorio::listar_modificaciones_ingresos($this->ano, $this->mes,  $this->tipo,  $this->ue);
        $foot = ['anulacion' => 0, 'credito' => 0];
        foreach ($body as $key => $value) {
            $foot['anulacion'] += $value->anulacion;
            $foot['credito'] += $value->credito;
        }
        return view("Presupuesto.Modificaciones.PrincipalIngresosTabla1Export", compact('body', 'foot'));
    }
    ...
    public function  registerEvents(): array
    {
        ...
        $opt = [AfterSheet::class => function (AfterSheet $event) use ($head, $foot, $body, $bodya, $bodyb, $border) {
            ...
            $event->sheet->getStyle('A1:H28')->applyFromArray($border);
        }];
