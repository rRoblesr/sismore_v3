@extends('layouts.main', ['titlePage' => 'SEGUIMIENTO DE NIÑOS'])
@section('css')
    <!-- Table datatable css -->
                        ...
                        <div class="row justify-content-between ">
                            <div class="col-md-4">
                                <div class="row form-group">
                                    ...
                                    <div class="col-md-6">
                                        <select class="form-control btn-xs" name="grupoEdad" id="grupoEdad" onchange="cargarListadoGrupoEdad();">
                                           @foreach ($grupo_edad as $item)
                                             <option {{ ($id_grupo==$item['id'])?'selected':''; }} value="{{ $item['id'] }}">{{ $item['nombre'] }}</option>
                                           @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="row justify-content-between ">
                            ...
                            <div class="col-md-3">
                                <div class="row form-group">
                                    ...
                                    <div class="col-md-8">
                                        <select class="form-control btn-xs" name="red" id="red" onchange="cargarListadoRed();">
                                           @foreach ($grupo_red as $item)
                                             <option {{ ($id_grupo==$item->cod_red)?'selected':''; }} value="{{ $item->cod_red }}">{{ $item->nom_red }}</option>
                                           @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="row form-group">
                                    ...
                                    <div class="col-md-8">
                                        <select class="form-control btn-xs" name="microred" id="microred" onchange="cargarListadoMicrored();">
                                           @foreach ($grupo_microred as $item)
                                             <option {{ ($id_grupo==$item->cod_mic)?'selected':''; }} value="{{ $item->cod_mic }}">{{ $item->nom_mic }}</option>
                                           @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-8">
                                        <select class="form-control btn-xs" name="ipress" id="ipress" onchange="cargarListadoGrupoIpress();">
                                           @foreach ($grupo_ipress as $item)
                                             <option {{ ($dato_ipress->cod_2000==$item->cod_2000)?'selected':''; }} value="{{ $item->cod_2000 }}">{{ $item->nom_est }}</option>
                                           @endforeach
                                        </select>
                                    </div>
                                </div>
         ...
         <div class="modal-dialog modal-lg">
          <div class="modal-content p-0">
           <ul class="nav nav-tabs nav-justified" role="tablist">
               <li class="nav-item">
                   <a class="nav-link active" id="home-tab-1" data-toggle="tab" href="#home-1" role="tab" aria-controls="home-1" aria-selected="true">
               ...
               <li class="nav-item">
                   <a class="nav-link" id="hemoglobina-tab-1" data-toggle="tab" href="#hemoglobina-1" role="tab" aria-controls="hemoglobina-1" aria-selected="false">
                       <span class="d-block d-sm-none"><i class="fa fa-user"></i></span>
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="microrred" placeholder="-" readonly>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
    ...
    <script>
        ...
        $(document).ready(function() {
            table_principal = $('#datatableS').DataTable({
                ...
                type: "POST",
                columnDefs: [
                    { targets: [5, 6, 7, 8, 9, 10, 11], className: 'text-center' },
                ]
            });
        });
        ...
        function completarDatosSeguimiento(data) {
            ...
            $('#tipo_doc').text(tipo_doc);    $('#num_doc').val(documento);         $('#nombre').val(data.nombre_nino);
            $('#fecha_nacimiento').val(data.fecha_nacimiento);     $('#edad').val(parseInt(data.edad_anio)*12+parseInt(data.edad_mes));
            $('#dni_madre').val(data.dni_madre);  $('#celular').val(data.celular);   $('#distrito').val(data.distrito);     $('#direccion').val(data.direccion);
            $('#microrred').val(data.microred);            $('#cod_eess').val(data.renaes);            $('#nom_eess').val(data.eess);
            $('#nombre_calidad').text(data.nombre_calidad);            $('#descripcion_calidad').text(data.descripcion_calidad);

            var dataCtrl = [
                { id: 1, fechaAtencion: data.f_credrn1, edadAtencion: data.edad_credrn1 }, { id: 2, fechaAtencion: data.f_credrn2, edadAtencion: data.edad_credrn2 },
                { id: 3, fechaAtencion: data.f_credrn3, edadAtencion: data.edad_credrn3 }, { id: 4, fechaAtencion: data.f_credrn4, edadAtencion: data.edad_credrn4 },
            ];
            var tableHtml = '<table class="table table-bordered mb-0">' + '<thead>' + '<tr>' + '<th>#</th>' + '<th>Fecha de Atención</th>' + '<th>Edad</th>' + '</tr>' + '</thead>' + '<tbody>';
            dataCtrl.forEach(function(item, index) {
                if(item.fechaAtencion!='-')
                    tableHtml += '<tr>' +'<td>' + item.id + '</td>' + '<td>' + item.fechaAtencion + '</td>' + '<td>' + item.edadAtencion + '</td>' + '</tr>';
            });
            tableHtml += '</tbody>' + '</table>';

            var dataCtrl = [
                { id: 1, fechaAtencion: data.f_cred01, edadAtencion: data.edad_cred01 }, { id: 2, fechaAtencion: data.f_cred02, edadAtencion: data.edad_cred02 },
                { id: 3, fechaAtencion: data.f_cred03, edadAtencion: data.edad_cred03 }, { id: 4, fechaAtencion: data.f_cred04, edadAtencion: data.edad_cred04 },
                { id: 5, fechaAtencion: data.f_cred05, edadAtencion: data.edad_cred05 }, { id: 6, fechaAtencion: data.f_cred06, edadAtencion: data.edad_cred06 },
                { id: 7, fechaAtencion: data.f_cred07, edadAtencion: data.edad_cred07 }, { id: 8, fechaAtencion: data.f_cred08, edadAtencion: data.edad_cred08 },
            ...
            var dataHemo = [
                { id: 1, fechaAtencion: data.f_hb1, resultados: data.Rhb1, edadAtencion: data.edad_hb1 }, { id: 2, fechaAtencion: data.f_hb2, resultados: data.Rhb2, edadAtencion: data.edad_hb2 },
                { id: 3, fechaAtencion: data.f_hb3, resultados: data.Rhb3, edadAtencion: data.edad_hb3 }, { id: 4, fechaAtencion: data.f_hb4, resultados: data.Rhb4, edadAtencion: data.edad_hb4 },
                { id: 5, fechaAtencion: data.f_hb5, resultados: data.Rhb5, edadAtencion: data.edad_hb5 }, { id: 6, fechaAtencion: data.f_hb6, resultados: data.Rhb6, edadAtencion: data.edad_hb6 },
            ];
            tableHtml = '<table class="table table-bordered mb-0">' + '<thead>' + '<tr>' + '<th>#</th>' + '<th>Fecha de Atención</th>' + '<th>Resultados</th>' + '<th>Edad</th>' + '</tr>' + '</thead>' + '<tbody>';
            dataHemo.forEach(function(item, index) {
                if(item.fechaAtencion!='-')
                    tableHtml += '<tr>' +'<td>' + item.id + '</td>' + '<td>' + item.fechaAtencion + '</td>' + '<td>' + item.resultados + '</td>' + '<td>' + item.edadAtencion + '</td>' + '</tr>';
            });
            tableHtml += '</tbody>' + '</table>';
            ...
            var dataSuple = [
                { id: 1, fechaAtencion: data.f_sup1, labSup: data.lab_sup1, edadAtencion: data.edad_sup1 }, { id: 2, fechaAtencion: data.f_sup2, labSup: data.lab_sup2, edadAtencion: data.edad_sup2 },
                { id: 3, fechaAtencion: data.f_sup3, labSup: data.lab_sup3, edadAtencion: data.edad_sup3 }, { id: 4, fechaAtencion: data.f_sup4, labSup: data.lab_sup4, edadAtencion: data.edad_sup4 },
                { id: 5, fechaAtencion: data.f_sup5, labSup: data.lab_sup5, edadAtencion: data.edad_sup5 }, { id: 6, fechaAtencion: data.f_sup6, labSup: data.lab_sup6, edadAtencion: data.edad_sup6 },
                { id: 7, fechaAtencion: data.f_sup7, labSup: data.lab_sup7, edadAtencion: data.edad_sup7 }, { id: 8, fechaAtencion: data.f_sup8, labSup: data.lab_sup8, edadAtencion: data.edad_sup8 },
                { id: 9, fechaAtencion: data.f_sup9, labSup: data.lab_sup9, edadAtencion: data.edad_sup9 }, { id: 10, fechaAtencion: data.f_sup10, labSup: data.lab_sup10, edadAtencion: data.edad_sup10 },
                { id: 11, fechaAtencion: data.f_sup11, labSup: data.lab_sup11, edadAtencion: data.edad_sup11 }, { id: 12, fechaAtencion: data.f_sup12, labSup: data.lab_sup12, edadAtencion: data.edad_sup12 },
            ];
            tableHtml = '<table class="table table-bordered mb-0">' + '<thead>' + '<tr>' + '<th>#</th>' + '<th>Fecha de Atención</th>' + '<th>Lab Supl.</th>' + '<th>Edad</th>' + '</tr>' + '</thead>' + '<tbody>';
             dataSuple.forEach(function(item, index) {
                if(item.fechaAtencion!='-')
                    tableHtml += '<tr>' +'<td>' + item.id + '</td>' + '<td>' + item.fechaAtencion + '</td>'+ '<td>' + item.labSup	+ '</td>' + '<td>' + item.edadAtencion + '</td>' + '</tr>';
            });
            tableHtml += '</tbody>' + '</table>';
            ...
            tableHtml = '<table class="table table-bordered mb-0">' + '<thead>' + '<tr>' + '<th>Vacuna</th>' + '<th>Fecha de Atención</th>' + '<th>Edad</th>' + '</tr>' + '</thead>' + '<tbody>';
             dataVacuna.forEach(function(item, index) {
                tableHtml += '<tr>' +'<td>' + item.vacuna + '</td>' + '<td>' + item.fechaAtencion + '</td>' + '<td>' + item.edadAtencion + '</td>' + '</tr>';
            });
            tableHtml += '</tbody>' + '</table>';
