    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            table_principal = $('#datatable').DataTable({
                ...
                ajax: "{{ route('imporpadronnominal.listar.importados') }}",
                type: "POST",
            });
        });
        ...
        function upload(e) {
            ...
            $.ajax({
                ...
                type: "POST",
                url: "{{ route('imporpadronnominal.guardar') }}",
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
        ...
        function geteliminar(id) {
            bootbox.confirm("¿Seguro desea eliminar esta importación?", function(result) {
                if (result === true) {
                    $.ajax({
                        url: "{{ route('imporpadronnominal.eliminar', '') }}/" + id,
                        type: "GET",
                        dataType: "JSON",
        ...
        function monitor(id) {
            $('#siagie-matricula').DataTable({
                ...
                ajax: {
                    ...
                    url: "{{ route('imporpadronnominal.listarimportados') }}",
                    type: "POST",
                    data: {
                        importacion_id: id // Pasa el ID de importación
                    }
                },
                columns: [{
                        data: 'padron',
