        ...
        <div class="col-md-12">
            <div class="card card-border">
                <div class="card-header border-success-0 bg-transparent pb-0">
                    ...
                    <h3 class="card-title">HISTORIAL DE IMPORTACIÓN - PADRÓN ANEMIA</h3>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12">
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#fuente2').on('change', function() {
                ...
                var fuenteSeleccionada = $(this).find('option:selected').text();

                // Buscar el único card-title dentro del card que contiene la tabla
                $('#datatable').closest('.card').find('.card-title').text(
                    'HISTORIAL DE IMPORTACIÓN - ' + fuenteSeleccionada);
            });

            // $('.bs-example-modal-lg').on('hidden.bs.modal', function() {
            //     console.log('X1');
            //     var selectedValue = $('#fuente-modal').val(); // Obtiene el valor seleccionado
            //     localStorage.setItem('fuenteSeleccionada',
            //         selectedValue); // Guarda el valor en localStorage
            // });

            $('.bs-example-modal-lg').on('shown.bs.modal', function() {
                console.log('Y1');
                var fuenteSeleccionada = $('#fuente2').val(); // localStorage.getItem('fuente2');
                if (fuenteSeleccionada) {
                    $('#fuente').val(fuenteSeleccionada);
                }
            });

            $('#fuente').on('change', function() {
                $('#fuente2').val(this.value);
                cargarhistorial();
            });
