<table id="tabla2tabla1" class="table table-sm table-striped table-bordered font-11 m-0" style="height: 100%">
    <thead>
        ...
        <tbody>
            @foreach ($base as $key => $item)
                <tr class="text-center">
                    <td>{{ $key + 1 }}</td>
                    <td class="text-left">
                        <a href="javascript:void(0)" onclick="cargarmicrored({{ $item->idmicro }})">{{ $item->micro }}
                        </a>
                    </td>
                    <td>{{ $item->den }}</td>
                    <td>{{ $item->num }}</td>
