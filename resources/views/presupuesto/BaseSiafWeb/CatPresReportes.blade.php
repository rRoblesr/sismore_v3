        ...
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-header bg-success-0">
                    ...
                    <h3 class="card-title text-white font-14">EJECUCIÓN DEL GASTO POR CATEGORIA PRESUPUESTAL</h3>
                </div>
        ...
        <div class="col-lg-12">
            <div class="card card-border border border-plomo-0">
                <div class="card-header border-success-0 bg-transparent pb-0 pt-2">
                    ...
                    <h3 class="card-title">
                        Ejecución del Gasto Presupuestal por Categoría Presupuestal
                    </h3>
                </div>
                <div class="card-body">
                    <div class="row">
...
@section('js')
    <script type="text/javascript">
        ...
        function panelGraficas(div) {
            $.ajax({
                url: "{{ route('presupuesto.catpres.reportes.reporte') }}",
                data: {
                    'div': div,
                    "anio": $('#anio').val(),
                    "ue": $('#ue').val(),
                ...
                success: function(data) {
                    switch (div) {
                        case 'anal1':
                            ...
                            $('#anal1_devengado').text(new Intl.NumberFormat('en-US').format(data.devengado)).counterUp({
                                delay: 10,
                                time: 1000
                            });
        ...
        function cargarPresupuesto() {
            $.ajax({
                url: "{{ route('presupuesto.saifweb.detalle.select.ff', ['anio' => ':anio', 'ue' => ':ue', 'cg' => ':cg']) }}"
                    .replace(':anio', $('#anio').val())
                    .replace(':ue', $('#ue').val())
                    .replace(':cg', $('#cg').val()),
                type: 'GET',
        ...
        function descargarExcel(div) {
            window.open(
                "{{ route('presupuesto.catpres.reportes.download.excel', ['div' => ':div', 'anio' => ':anio', 'ue' => ':ue', 'cg' => ':cg', 'ff' => ':ff', 'cp' => ':cp']) }}"
                .replace(':div', div)
                .replace(':anio', $('#anio').val())
                .replace(':ue', $('#ue').val())
                .replace(':cg', $('#cg').val())
                .replace(':ff', $('#ff').val())
                .replace(':cp', select_cp),
            );
        }
