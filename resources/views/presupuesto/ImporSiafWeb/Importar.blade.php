@extends('layouts.main', ['titlePage' => 'IMPORTAR DATOS - GASTO SIAF WEB'])
@section('css')
    <!-- Table datatable css -->
                    ...
                    <div class="card-header border-success-0 bg-transparent pb-0">
                        <div class="card-widgets">
                            ...
                            <button type="button" class="btn btn-success btn-xs"
                                onclick="javascript:window.open('https://docs.google.com/spreadsheets/d/1It2kMDkpl8SIClqBLJwFPwoRgdixQ_Z-/edit?usp=sharing&ouid=108127328164905589197&rtpof=true&sd=true','_blank');"><i
    ...
    <script>
        ...
        function upload(e) {
            ...
            $.ajax({
                ...
                type: "POST",
                url: "{{ route('imporsiafweb.guardar') }}",
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
        ...
        function geteliminar(id) {
            bootbox.confirm("Seguro desea Eliminar este IMPORTACION?", function(result) {
                if (result === true) {
                    $.ajax({
                        url: "{{ url('/') }}/IMPORSIAFWEB/eliminar/" + id,
                        type: "GET",
                        dataType: "JSON",
                        beforeSend: function() {
                            $('#eliminar' + id).html(
