...
@section('js')
    ...
    <script>
        ...
        function listarDT() {
            if ($('#bentidad').val() > 0 && $('#bgerencia').val() > 0) {
        ...
        function cargarGerencia(id) {
            $.ajax({
                url: "{{ route('entidad.ajax.cargar') }}",
                data: {
                    "dependencia": $('#bentidad').val()
                },
                type: 'get',
                success: function(data) {
                    $("#bgerencia option").remove();
        ...
        function edit(id) {
            ...
            $('#id').val(id);
            $.ajax({
                url: "{{ route('entidad.ajax.edit.oficina', '') }}/" + id,
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    $("#entidad").val(data.oficina.entidad);
                    $("#descripcion").val(data.oficina.nombre);
                    $("#apodo").val(data.oficina.apodo);

                    cargarGerencia2(data.oficina.dependencia);
                    $('#modal_form_entidad').modal('show');
