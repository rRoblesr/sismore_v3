...
@section('js')
    ...
    <script>
        ...
        function edit(id) {
            ...
            $('#id').val(id);
            $.ajax({
                url: "{{ route('entidad.ajax.edit.gerencia', '') }}/" + id,
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    $("#dependencia").val(data.gerencia.dependencia);
                    $("#descripcion").val(data.gerencia.nombre);
                    $("#abreviado").val(data.gerencia.abreviado);
