<table id="tabla1" class="table table-striped table-bordered tablex" style="font-size:11px;">
    <thead>
        <tr class="bg-success-0 text-white text-center">
            ...
            <th colspan="2" class="text-center">Intermedio</th>
            <th colspan="2" class="text-center">Avanzado</th>
        </tr>
        <tr class="bg-success-0 text-white text-center">
    ...
    @if ($base->count() > 0)
        <tbody>
            @foreach ($base as $item)
                <tr class="text-center">
                    <td class="text-left"><a href="javascript:void(0)">{{ $item->distrito }}</a></td>
        ...
        <tfoot>
            <tr class="text-center bg-success-0 text-white">
                ...
                <th class="text-center">{{ number_format($foot->thi, 0) }}</th>
                <th class="text-center">{{ number_format($foot->tmi, 0) }}</th>
                <th class="text-center">{{ number_format($foot->thp, 0) }}</th>
                <th class="text-center">{{ number_format($foot->tmp, 0) }}</th>
                <th class="text-center">{{ number_format($foot->ths, 0) }}</th>
                <th class="text-center">{{ number_format($foot->tms, 0) }}</th>
            </tr>
        </tfoot>
    @else
        <tboy>
